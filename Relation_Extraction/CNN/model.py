...
class Attention_CNN(nn.Module):
    ...
    def forward(self, data):
        # Step 1: 数据预处理
        #从输入数据中提取各个部分，包括token、位置1、位置2、掩码、标签、实体1掩码和实体2掩码
        ...
        # Step 2: 编码层
        # 调用encoder_layer函数将token、位置和标签转换为嵌入表示
        emb = self.encoder_layer(token, pos1, pos2, tags)
        # Step 3: 卷积层
        # 调用conv_layer函数对嵌入表示进行卷积操作，并通过tanh激活函数进行非线性变换
        conv = self.conv_layer(emb, mask)
        conv = self.tanh(conv)
        # Step 4: 池化层
        # 调用single_maxpool_layer函数对卷积后的特征进行最大池化
        pool = self.single_maxpool_layer(conv)
        # Step 5: 实体平均和注意力层
        # 调用entity_average函数计算实体1和实体2的平均嵌入表示
        # 调用attention_layer函数计算实体1和实体2的上下文表示
